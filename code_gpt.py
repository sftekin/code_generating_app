...
class CodeGPT(AbstractFunction):
    """
    Arguments:
    ...
    Example Usage:
        ...
        If the user wants to retrieve the summary of each video, the CodeGPT UDF can be used as:

            query = "Generate the summary of the video"
            cursor.table("video_transcripts").select(f"CodeGPT({question}, text)")
        ...
        along with each query, a prompt can be set as such:
            ...
            cursor.table("video_transcripts").select(f"CodeGPT({question}, text, {prompt})")
    ...
    def forward(self, text_df):
        ...
        from gpt_engineer.cli.main import preprompts_path
        ...
        all_output_path = workspace_path / "all_output.txt"
        dbs = DBs(
            memory=DB(memory_path),
            logs=DB(memory_path / "logs"),
            input=DB(input_path),
            workspace=DB(workspace_path),
            preprompts=DB(preprompts_path(False, input_path)),
